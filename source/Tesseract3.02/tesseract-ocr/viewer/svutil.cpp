...
void SVNetwork::Flush() {
  ...
  while (msg_buffer_out_.size() > 0) {
    int i = 0/*send(stream_, msg_buffer_out_.c_str(), msg_buffer_out_.length(), 0)*/;
	#pragma message("项目需要, 人为注释, http://blog.csdn.net/rrrfff")
...
char* SVNetwork::Receive() {
  ...
  // Otherwise, we read from the stream_.
    ...
    i = 0/*recv(stream_, msg_buffer_in_, kMaxMsgSize, 0)*/;
	#pragma message("项目需要, 人为注释, http://blog.csdn.net/rrrfff")
...
// Get the host data depending on the OS.
  ...
  *addr_info = new struct addrinfo;
  ...
  name = NULL/*gethostbyname(hostname)*/;
  #pragma message("项目需要, 人为注释, http://blog.csdn.net/rrrfff")
#else
...
SVNetwork::SVNetwork(const char* hostname, int port) {
  ...
  if (false/*connect(stream_, addr_info->ai_addr, addr_info->ai_addrlen) < 0*/) {
	#pragma message("项目需要, 人为注释, http://blog.csdn.net/rrrfff")
...
#undef _STR
	...
	while (false/*connect(stream_, (struct sockaddr *) addr_info->ai_addr, addr_info->ai_addrlen) < 0*/) {
      #pragma message("项目需要, 人为注释, http://blog.csdn.net/rrrfff")
